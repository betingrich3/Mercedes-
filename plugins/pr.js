...
async (conn, mek, m, { from, prefix, quoted, q, reply }) => {
    try {
        ...
        // Validate input
        if (!q) {
            return await reply("*Example -* .pair +25474000****");
        }

        // Fetch pairing code
        const fetch = require("node-fetch");
        const response = await fetch(`https://pairl-f69c1fb8884c.herokuapp.com/pair?number=${q}`);
        ...
        const doneMessage = "> *Mercedes Is Connected*";
